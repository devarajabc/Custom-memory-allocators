...
static bool size_lessthan(const rb_node_t *a, const size_t b)
{
    if (!a || !b)
        return false;
    const blockmark_t *block_a = container_of(a, blockmark_t, node);
    return SIZE_BLOCK(block_a->next) < b;
}
...
static void* createAlignBlock(rb_t* tree, void *sub, size_t size)
{
...
static int expandBlock(rb_t* tree, blockmark_t* sub, size_t newsize, void** pstart)
{
    blockmark_t *s = sub;
    blockmark_t *n = NEXT_BLOCK(s);
    ...
    if((s->next.offs+n->next.offs)-newsize<THRESHOLD+2*sizeof(blockmark_t)) {
        ...
        blockmark_t *next = NEXT_BLOCK(n);// thenext of old n.  |s|.    |n|.  |m|   |next|
...
void internal_customFree(void* p, int is32bits)
{
    ...
    if(l) {
        if(l->type==BTYPE_LIST) {
            ...
            #ifdef TRACE_MEMSTAT
            else printf(LOG_INFO, "Warning, customme free(%p) from MAP block %p, but not found as allocated\n", p, l);
            #endif
            if(l->lowest>idx)
                l->lowest = idx;
            return;
...
void* internal_customMemAligned(size_t align, size_t size, int is32bits)
{
    ...
    p_blocks[i].size = allocsize;
    #ifdef TRACE_MEMSTAT
    printf(LOG_INFO, "Custommem: allocation %p-%p for LIST Alloc p_blocks[%d], aligned\n", p, p+allocsize, i);
    #endif
    // setup marks
    blockmark_t* m = (blockmark_t*)p;
    m->prev.x32 = 0;
...
void* customMemAligned32(size_t align, size_t size)
{
    ...
    if(((uintptr_t)ret)>=0x100000000LL) {
        printf("Error, customAligned32(0x%lx, 0x%lx) return 64bits point %p\n", align, size, ret);
    }
    return ret;
}
